...
class AddCaregiver : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        supportActionBar?.title = "إضافة مسؤول رعاية"

    AddNewCaregiver.setOnClickListener {
...
///This page is for the Child to add another caregiver

private fun updateChildrenEmails(){

    val currentUs = FirebaseAuth.getInstance().currentUser

    val ref = FirebaseDatabase.getInstance().getReference("/users/${currentUs!!.uid}").child("cgemail")
    ...
    ref.addListenerForSingleValueEvent(object : ValueEventListener {
        ...
        override fun onDataChange(p0: DataSnapshot) {
            ...
            listOfChildrenss.add(editTextCaregiverEmail.text.toString()) //get the new email

            ref.setValue(listOfChildrenss) //set the value of the new email + already existing emails
                .addOnCompleteListener {

                    Toast.makeText(this@AddCaregiver, "تم إضافة مسؤول الرعاية", Toast.LENGTH_LONG).show()// when insertion is successful show this toast
                }
        }
