...
import kotlinx.android.synthetic.main.activity_view_children_results.*
import kotlinx.android.synthetic.main.user_row.view.*

class ViewChildrenResults : AppCompatActivity() {

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.activity_view_children_results)

        supportActionBar?.title = "نتائج الأطفال"
            ...
            ref.addListenerForSingleValueEvent(object : ValueEventListener { //function listenter to help retrieve the data
                ...
                override fun onDataChange(p0: DataSnapshot) {
                    ...
                    p0.children.forEach {
                        ...
                        if (user != null && themail in user.specialist && user.usertype=="طفل") {
                            adapter.add(UserchildItem(user)) // if the user is type 'طفل' and linked to this specialist add the user to the adapter
                        }
                    }

                    adapter.setOnItemClickListener { item, view ->

                        val userItem = item as UserchildItem // set the user that i passed to UserchildreportItem to userItem variable

                        val intent = Intent(this@ViewChildrenResults, ViewResultsByDate::class.java) // when the specialist clicks any user it will move the page to ViewResultsByDate
//          intent.putExtra(USER_KEY,  userItem.user.username)
                    ...
                    ChildrenList_RecyclerView.adapter = adapter
...
class UserchildItem(val user: User): Item<ViewHolder>() { //class to take the passed 'user'and put its info in a view Holder
